...
| file | role |
|---|---|
| `constraints.py` | constraint library (28 constraints, verifiers, transforms) |
| `seed_datasets.py` | load + normalize 5 seed datasets; question formatting + answer checking |
| `build_pairs.py` | sample (question, constraint) pairs; train/val split |
| `qwen_rollouts.py` | run base Qwen on prompts → `<think>` reasoning + answer |
| `cot_judge.py` | score CoT-controllability (det. verify + GPT-4.1-mini judges) |
| `make_sft_data.py` | GPT-4.1-mini rewrites rollouts → compliant SFT targets |
| `openai_utils.py` | GPT-4.1-mini chat wrapper (`OPENAI_API_KEY`) |
...
python build_pairs.py --n_train 1000 --n_val 120

# 2. Qwen rollouts on VAL first (baseline material + sanity check)
python qwen_rollouts.py --pairs data/pairs.val.jsonl --out data/rollouts.val.jsonl

# 3. judge the val baseline (how good is Qwen at controllability BEFORE training?)
python cot_judge.py --rollouts data/rollouts.val.jsonl --out data/judged.val.jsonl

# 4. once happy, rollouts on train, then rewrite both splits into SFT data
python qwen_rollouts.py --pairs data/pairs.train.jsonl --out data/rollouts.train.jsonl
python make_sft_data.py --rollouts data/rollouts.train.jsonl --out data/sft.train.jsonl
python make_sft_data.py --rollouts data/rollouts.val.jsonl   --out data/sft.val.jsonl
```
...
- **Offline proxy:** evaluate trained checkpoints on `heldout_benchmark`
  ...
  CoT-Control suite.
- **Training:** `train_sft.py` (TRL `SFTTrainer`, Qwen chat format, LoRA/QLoRA/full),
  `merge_lora.py`, `train.sh`, `requirements-train.txt`.
