...
# Train rollouts feed cotctrl/make_sft_data.py (gpt-4.1-mini rewrites them to
# satisfy each constraint -> SFT targets). val/test rollouts are the baseline.
...
[ -f data/pairs.train.jsonl ] || uv run python cotctrl/build_pairs.py --n_train 1000 --n_val 150 --n_test 250

log "STEP 2: rollouts (skip any that already exist)"
[ -f data/rollouts.train.jsonl ] || uv run python cotctrl/qwen_rollouts.py --pairs data/pairs.train.jsonl --out data/rollouts.train.jsonl --batch_size $BS
[ -f data/rollouts.val.jsonl ]   || uv run python cotctrl/qwen_rollouts.py --pairs data/pairs.val.jsonl   --out data/rollouts.val.jsonl   --batch_size $BS
[ -f data/rollouts.test.jsonl ]  || uv run python cotctrl/qwen_rollouts.py --pairs data/pairs.test.jsonl  --out data/rollouts.test.jsonl  --batch_size $BS
...
log "  baseline judged: data/judged.{val,test}.jsonl  (summarize: uv run python cotctrl/analyze.py data/judged.val.jsonl)"
log "  next: run_finetune.sh (builds SFT data from train rollouts, trains, evals)"
